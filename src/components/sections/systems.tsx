      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mb-8 md:mb-10">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <p className="max-w-sm text-sm leading-relaxed text-ink-hi0">
          ...
          {SYSTEMS.map((sys, i) => {
            ...
            return (
              <motion.article
                ...
                className="group flex h-full cursor-pointer flex-col rounded-lg border border-line bg-surface-card p-4 transition-colors hover:border-cyan-500/30 hover:bg-surface-raised focus-visible:outline focus-visible:outline-2 focus-visible:outline-cyan-400/60 md:p-5"
              >
                {/* Top row: index / category · badge · code icon */}
                <div className="mb-4 flex items-center justify-between gap-2">
                  <span className="truncate font-mono text-[10px] font-semibold tracking-[0.18em] text-ink-mid">
                    <span className="text-ink-faint">{sys.index} / </span>
                    {sys.category}
                  </span>
                  <span className="flex shrink-0 items-center gap-2">
                    <span
                      className={`rounded px-2 py-0.5 font-mono text-[9px] font-semibold tracking-[0.18em] ${
                        production
                          ...
                          : "border border-zinc-700 text-ink-mid"
                      }`}
                    >
                      {sys.badge}
                    </span>
                    <a
                      ...
                      className="font-mono text-xs text-ink-faint transition-colors hover:text-cyan-300"
                    >
                      {"</>"}
                    </a>
                  </span>
                ...
                {/* Title + description */}
                <h3 className="text-base font-bold leading-snug tracking-tight text-ink-hi md:text-lg">
                  {sys.title}
                </h3>
                <p className="mt-1.5 text-xs leading-relaxed text-ink-hi0">
                  ...
                  <div className="flex items-end justify-between gap-3 border-t border-line pt-3">
                    {sys.stat ? (
                      <p className="flex items-baseline gap-2 leading-none">
                        ...
                        <span className="font-mono text-[10px] leading-tight text-ink-hi0">
                          {sys.stat.label}
                        </span>
                      </p>
                    ) : (
                    ...
                    <span className="flex shrink-0 items-center gap-1.5 font-mono text-[10px] font-semibold tracking-[0.18em] text-ink-hi0 transition-colors group-hover:text-cyan-300">
                      OPEN CASE STUDY
                      <ArrowRight
                        size={12}
        ...
        <p className="mt-8 text-center font-mono text-xs text-ink-faint">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="text-ink-mid underline decoration-zinc-700 underline-offset-4 hover:text-cyan-300"
          >
